...
def school_center():
    ...
    while Flag:
        ...
        if school_name in res_dict:
            ...
            while Flag:
                print("\33[32;1m欢迎进入%s学校\33[0m".center(50, "*") % school.name)
                choice = options(list_school)  # 打印当前选项
                if choice == "1":
                    while True:
                        print("\33[32;0m学校:%s目前已经有的班级信息\33[0m".center(40, "-") % school.name)
                        ...
                        if if_cont == "y":
                            ...
                            if course_name in res_course:
                                ...
                                print("\33[31;0m错误:课程:%s不存在，请先创建课程\33[0m" % course_name)
                        if if_cont == "b":
                            break
                if choice == "2":
                    # 招聘讲师
                    while True:
                        print("\33[32;0m学校:%s目前已经有的课程与讲师\33[0m".center(40, "-") % school.name)
                        res_course = information(dict_main[school], "None")[0]
                        set_info = set([])
                        if res_course:  # 打印课程与讲师对应关系
                            for i in res_course:
                        ...
                        if if_cont == "y":
                            ...
                            if course_name in res_course:
                                ...
                                if teacher_name not in res_teacher:
                                    ...
                                    print("\33[31;0m错误:教师:%s已经被聘用\33[0m" % teacher_name)
                            else:
                                print("\33[31;0m错误:课程:%s不存在，请先创建课程\33[0m" % course_name)
                        if if_cont == "b":
                            break
                if choice == "3":
                    # 创建课程
                    while True:
                        print("\33[32;0m学校:%s目前已经有的课程\33[0m".center(40, "-") % school.name)
                        res_dict = information(dict_main[school], "course")[0]  # 打印课程信息赋值给字典course_dict
                        if_cont = input("\n\33[34;0m是否要创建课程y创建b退出\33[0m:")
                        if if_cont == "y":
                            course_name = input("\33[34;0m输入要创建的课程\33[0m:").strip()
                            if course_name not in res_dict:  # 课程不存在，创建
                                price = input("\33[34;0m输入课程:%s的价格\33[0m:" % (course_name))
                                time = input("\33[34;0m输入课程:%s的周期（月）\33[0m:" % (course_name))
                                course = Course(course_name, price, time)  # 创建课程course
                                school.create_course(dict_main, course, __db_main)  # 关联学校和课程
                            else:  # 课程存在
                                print("\33[31;0m错误：当前课程:%s已经存在\33[0m" % (course_name))
                        if if_cont == "b":
                            break
                if choice == "4":
                    Flag = False
        if Flag:
            print("\33[31;0m错误：输入的学校%s不存在\33[0m" % (school_name))
...
def teacher_center():
    ...
    if teacher_name in dict_info:
        while True:
            print("\33[32;1m欢迎进入讲师%s的管理中心\33[0m".center(40, "*") % teacher_name)
            choice = options(list_teacher)
            teacher = dict_info[teacher_name]
            grade = teacher_dict[teacher]["grade"]
            if choice == "1":
                print("\33[32;0m讲师:%s的班级信息\33[0m".center(40, "-") % teacher.name)
                print("学校:%s\t课程:%s\t班级:%s\t" % (teacher.school, teacher.course, grade.name))
                any = input("\n\33[34;0m输入任意键退出当前\33[0m:")
            if choice == "2":
                print("\33[32;0m讲师:%s的班级学员列表\33[0m".center(40, "-") % teacher.name)
                print("班级:%s\n学员:%s" % (grade.name, grade.student))
                any = input("\n\33[34;0m输入任意键退出当前\33[0m:")
            if choice == "3":
                break
    else:
        print("\33[31;0m错误：讲师:%s不存在\33[0m" % (teacher_name))
                        ...
                        if dict[school][course].get("grade"):
                            for i in teacher_dict:
                                if course.name == i.course:
                                    teacher = i
                                    grade = teacher_dict[teacher]["grade"]
                            print("课程:%s的费用为:%s" % (course.name, course.price))
                            if_pay = input("\33[34;0m是否支付当前费用 支付y\33[0m:")
...
def start():
    '''程序开始'''
    while True:
        print("\33[35;1m欢迎进入老男孩教育选课系统\33[0m".center(50, "#"))
